...
assign stop_frame = counter[5] & ~counter[4] & ~counter[3] & ~counter[2] & ~counter[1] & ~counter[0] & mdio_start;
	  ...
	  begin
		if(~send_preamb)
		   mdio_out <= 1'b1;
			...
			mdio_out <= so | ~busy_d | data_bits & mdio_rw | start_frame_d;
	  end
    ...
    always @(posedge synced_clock)
    begin
        //cs_addr <= { 1'b0, start_frame | tc & ~data_bits & ~mdio_rw, counter[0] & busy | tc & mdio_rw }; 
		cs_addr <=  1'b0; 
    end
...
), .cy_dpconfig_b(
{
    ...
    `SC_WRK16CAT_DSBL /*CFG17-16:                          */
        ...
        /*  input                   */  .d0_load(1'b0),
        /*  input                   */  .d1_load(1'b0),
        /*  output  [01:00]         */  .ce0({ce0}),   /* Read Bits + PHY Address is stored in ce0 */
        /*  output  [01:00]         */  .cl0(),
        /*  output  [01:00]         */  .z0(),
        /*  output  [01:00]         */  .ff0(),
        /*  output  [01:00]         */  .ce1({ce1}),  /* Write Bits + PHY Address is stored in ce1 */
        /*  output  [01:00]         */  .cl1(),
        /*  output  [01:00]         */  .z1(),
        /*  output  [01:00]         */  .ff1(),
        /*  output  [01:00]         */  .ov_msb(),
